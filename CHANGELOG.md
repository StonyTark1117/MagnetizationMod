...
### New content
- **Homopolar Motor / Magnetic Flywheel**: a Create **kinetic generator that runs on a magnet alone** — no fuel, no energy. Right-click it with a magnet (Magnetic Plate / Temporary / Permanent Magnet) to install it and it spins a shaft on its placement axis; empty-hand right-click pops the magnet back out. Speed and stress capacity scale with the magnet's strength, but it's deliberately gentle (a Permanent Magnet tops out at 32 RPM / 128 su) — a compact, silent starter generator. (A formal magnet-slot GUI is a tracked follow-up; for now it's right-click insert.)
