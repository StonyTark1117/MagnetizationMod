...
- **Ferrofluid**: a black, water-like magnetic liquid (renders as near-black tinted water). Generates naturally **only in the Magnetic Anomaly biome** — its water bodies are ferrofluid — and is otherwise player-made (water bucket + 2 raw magnetite). Full fluid: source/flowing, placeable, bucketable. (Its reactive-to-fields / magnetize-to-polarize behavior is a tracked follow-up.)
...
- **Directional Repulsor + Vector Core**: craft a **Vector Core** (titanomagnetite + compass + redstone) and right-click a Repulsor Coil to install it. The coil then *thrusts* magnetic ships along its facing — a self-propelling magnetic conveyor/track — on top of its normal repulsion, up to a terminal track speed. Direction = the coil's facing (rotatable with a wrench); break the coil to recover the chip. (Direction is set by facing rather than GUI buttons — cleaner, and the coil's already rotatable; can add explicit direction buttons later if wanted.)
