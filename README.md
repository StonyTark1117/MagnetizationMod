...
Current release: **1.4.2**. This compatibility release connects Magnetization fields to Coasters: Magnetized anchors, adds a Magnetic Alloy route for Coasters: Engineered, verifies Coasters Extras against the parent cart contract, and lets EMP Flux Charges disable CBC Aeronautics Missile guidance. Every bridge targets the individual addon rather than a particular modpack.
...
- [Just Enough Resources](https://www.curseforge.com/minecraft/mc-mods/just-enough-resources-jer), a JEI add-on, adds synchronized distribution and drop charts for every Magnetization ore family, Helium-3 geodes, magnetic gravel, anomaly lodestone clusters, meteorites, petrified wood, and natural Helium/Radon pockets. `compat.justEnoughResourcesCompatEnabled` disables its registration.
