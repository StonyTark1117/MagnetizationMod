...
Current release: **1.4.5**. This release extends magnetic material recognition to Create: Ironworks and verifies Create: Coasters Simulated Additions through the existing coaster cart bridge. Both addons remain optional.
...
- [Jade](https://modrinth.com/mod/jade), [WTHIT](https://modrinth.com/mod/wthit), or [The One Probe](https://modrinth.com/mod/the-one-probe) for live HUD information on Magnetization machines and emitters. Each viewer has its own master compatibility switch. With Jade on a Sable craft, Sable owns sublevel-aware targeting and Magnetization owns the field/machine lines, so the information is not registered twice.
...
- [Just Enough Resources](https://www.curseforge.com/minecraft/mc-mods/just-enough-resources-jer), a JEI add-on, adds 28 synchronized distribution and drop charts covering every Magnetization ore variant, Helium-3 geodes, magnetic gravel, anomaly lodestone clusters, meteorites, petrified wood, and natural Helium/Radon pockets. `compat.justEnoughResourcesCompatEnabled` disables its registration.
...
- [Patchouli](https://modrinth.com/mod/patchouli) — adds a craftable in-game guide covering basics, emitters, machines, fluids, propulsion, compatibility, and the complete rare-earth progression. `compat.patchouliCompatEnabled` gates the book registration, recipes, and automatic gift; restart after re-enabling it. The Book + Raw Magnetite and Book + Lodestone recovery recipes are enabled by default and independently configurable. The generic Book + Iron Ingot alternative defaults off because it may conflict with another mod's Patchouli guide-book recipe.
- [Create: AeroPortals](https://www.curseforge.com/minecraft/mc-mods/aeroportals) — transfers Sable ships across dimensions and refreshes their derived magnetic state after traversal. AeroPortals 1.2.3+ also exposes the geometry needed to remap paired Railgun Remotes and Simulated swivel links; older 1.1.2 retains the safe base transfer behavior without attempting unavailable remapping calls. `compat.aeroPortalsCompatEnabled` disables the bridge. Verified against 1.1.2 and 1.3.0.
- [Immersive Aeronautics](https://www.curseforge.com/minecraft/mc-mods/immersive-aeronautics) — preserves and refreshes a Sable ship's magnetic state when Immersive Portals moves it between dimensions. `compat.immersivePortalsCompatEnabled` disables cross-portal force and state projection. Verified against the 1.1.4 package and Immersive Portals Core 6.0.7.
...
- [Create: Coasters Simulated](https://www.curseforge.com/minecraft/mc-mods/create-coasters-simulated) — coaster cars react to magnetic fields like other Sable craft. The Structural Inducer recognizes rail-engaged coaster trains (including linked cars and riveted bodies) as one existing structure, while loose carts are ignored. `compat.simulatedCoastersCompatEnabled` is the master above the two independent behavior toggles. The base loose-cart contract is verified against the pack-pinned 0.1 release as well as the current published runtime.
  Track Control Fins from Coasters Simulated Additions count as magnetic emitters by default. `compat.coastersAdditionsCompatEnabled` disables its integration, while `compat.coasterFinMagnetizationEnabled` disables only the Fin effect; both take effect on data reload.
- [Create: Coasters Simulated Additions](https://www.curseforge.com/minecraft/mc-mods/create-coasters-simulated-additions) — coaster cars, cladding, and seats keep the base Coasters magnetic and Structural Inducer behavior. Its Track Control Fin extends Simulated's Redstone Magnet and counts as an onboard magnetic emitter; lift/current paths remain owned by the addon. All entries are optional, with no addon-specific mixin or hard dependency.
- [Coasters Simulated: Track Styles](https://www.curseforge.com/minecraft/mc-mods/coasters-simulated-track-styles) — custom track geometries and independent anchor rail/spine colors coexist with the Coasters magnetic bridge. Styled circuits keep the same cart field reaction and Structural Inducer behavior; Magnetization also supplies the empty model omitted from the addon's 1.0.0 client resources. No extra toggle is needed because the addon adds presentation metadata rather than a new vehicle type. Verified against 1.0.0.
- [Coasters: Magnetized](https://www.curseforge.com/minecraft/mc-mods/coasters-magnetized) — a live Magnetization field can power Magnetized anchors as an alternative to redstone. The addon's selected mode, force, speed limit, native redstone behavior, and powered visuals stay authoritative; redstone plus field power never applies acceleration twice. The master and field-power switches both default on. Verified against 1.1.0.
- [Coasters: Engineered](https://www.curseforge.com/minecraft/mc-mods/coasters-engineered) — adds a condition-gated Linear Motor recipe using Magnetization Magnetic Alloy Plates. Engineered anchors retain their native bidirectional NeoForge FE contract. The master and recipe switches both default on. Verified against 1.0.1.
- [Create: Coasters Extras](https://www.curseforge.com/minecraft/mc-mods/create-coasters-extras) — functional and decorative tracks continue to use Coasters Simulated's cart contract, so carts on Extras track receive the existing magnetic-field and Structural Inducer behavior without an addon-specific gameplay override. Verified against 1.2.
- [Create: Ironworks](https://www.curseforge.com/minecraft/mc-mods/create-ironworks) — copper, bronze, brass, steel, and Sturdy armor participate in Magnetization's metal-armor effects. Tin/bronze/steel nuggets and tin/bronze sheets are ferromagnetic, and Sturdy Sheet Blocks contribute ship susceptibility. Rose Quartz armor is excluded. Verified against 4.0.3.
  `compat.ironworksCompatEnabled` disables this optional material/armor integration; changes take effect on data reload.
- [CBC Aeronautics Missiles](https://www.curseforge.com/minecraft/mc-mods/create-simulated-missiles-create-aeronautics) — an EMP Flux Charge permanently disables the guidance computer of each intersecting Sable missile. Propulsion, payloads, fuzes, and ordinary physics are untouched; both the integration and its guidance effect can be disabled. Verified against 0.1.0.
...
- [Ore Excavation](https://www.curseforge.com/minecraft/mc-mods/ore-excavation) — bulk-excavates Magnetization ore deposits and Helium-3 geodes while preserving Ore Excavation's configured tools, blacklists, range, and excavation limits. `compat.oreExcavationCompatEnabled` disables the bridge.
- [Create Ore Excavation](https://www.curseforge.com/minecraft/mc-mods/create-ore-excavation) — adds condition-gated infinite-vein recipes for every Magnetization ore family—Magnetite, Maghemite, Pyrrhotite, Hematite, Titanomagnetite, Lithium, Bastnäsite, Monazite, Cobaltite, and Borax—plus Helium-3. The recipes remain absent when the addon or shared Ore Excavation compatibility setting is disabled; the four rare-earth veins also have independent, default-on switches for progression-sensitive packs.
...
## New in 1.4.3 — Pack compatibility and mounted-Railgun hotfix

- **Mounted Railguns** have direct release-gate coverage for both open reports: a gun on a Sable sublevel fires an external staged payload, and an automatic gun ignores ferromagnetic blocks belonging to its own host craft.
- **AeroPortals 1.1.2**, **Create: Coasters Simulated 0.1**, and **JEI 19.27.0.336** are accepted and tested because those are the real individual-mod versions pinned by supported packs. Newer APIs remain available when newer addon versions are installed.
- Isolated client gates cover Coasters: Magnetized, Coasters: Engineered with Jade, Coasters Extras, CBC Aeronautics Missiles, and the complete 28-chart JER catalog.
- Pack manifests remain development-only fixtures. Magnetization never detects or depends on either pack, so custom packs receive the same integrations from the individual mod IDs.
...
- **Machine rebalance** — the Tokamak now burns three fuel-cell tiers and supports configurable odd-edged structures from 3x3 through 15x15 by default: a Tokamak-Coil perimeter must surround a solid Reactor-Core interior (one core at 3x3, nine at 5x5, twenty-five at 7x7), scaling capacity, generation, and output with size. The MHD Jet needs a conductive working fluid (gallium/mixed gallium/liquid lithium); the Micro-Thruster accepts magnetized ferrofluid; and magnet-slot machines can consume their magnet over a strength-scaled burn time (disabled by default, toggleable). Every fuel-burning machine accepts hopper/pipe fuel intake.
...
The rare-earth progression contract for this release is in [PROPOSED_1.4.0.md](PROPOSED_1.4.0.md).
...
./gradlew smokeOptionalClientProfiles # isolated HUD/viewer/Ponder/coaster/missile client boots
