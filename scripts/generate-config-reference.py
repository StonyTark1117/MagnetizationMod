...
def parse_spec() -> list[dict[str, str]]:
    ...
    sections: dict[str, list[str]] = {"b": [], "sb": [], "cb": []}
    owners: dict[str, str] = {}
    ...
    while i < len(lines):
        ...
        if push:
            ...
            for lookahead in lines[i + 1:]:
                next_assignment = re.search(r"^\s*[A-Z][A-Z0-9_]*\s*=\s*(b|sb|cb)(?:\s|\.|$)", lookahead)
                if next_assignment:
                    owner = next_assignment.group(1)
                    break
            owners[section] = owner
            sections[owner].append(section)
        pop = re.search(r"\b(b|sb|cb)\.pop\(\)", line)
        if pop and sections[pop.group(1)]:
            sections[pop.group(1)].pop()

        assignment = re.search(r"^\s*([A-Z][A-Z0-9_]*)\s*=\s*(b|sb|cb)(?:\s|\.|$)", line)
        if not assignment:
            i += 1
            continue
        ...
        values.append({
            "scope": {"b": "COMMON", "sb": "SERVER", "cb": "CLIENT"}[builder],
...
def render(values: list[dict[str, str]], lang: dict[str, str]) -> str:
    ...
    out = [
        ...
        "",
        "This is the complete reference for the live configuration spec. COMMON, SERVER, and",
        "CLIENT values are stored in config/magnetization-common.toml, the world's serverconfig/",
        "magnetization-server.toml, and config/magnetization-client.toml respectively. Defaults",
        "and ranges are taken from MagConfig.java; descriptions come from the English translation",
        "catalog.",
        "",
