"""Magnetite tool + armor textures, recolored from vanilla iron equivalents.

Uses the same darkening + steely-blue tint as the magnetite ingot/block textures
so the whole tier reads as a coherent set.
"""
...
def recolor(img: Image.Image) -> Image.Image:
    """Same shift as the ingot/block: darken with slight blue retention."""
    out = img.copy()
    pixels = out.load()
    w, h = out.size
    for y in range(h):
