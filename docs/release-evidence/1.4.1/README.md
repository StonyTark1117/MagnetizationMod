...
| Gas Detector HUD | [active scan](gas-detector-hud.png), [expanded Radon readout](../../../playtest/baselines/lab/gas_detector.png) | Active scans render state, heading, range, exposure, dose, and the safety verdict; the expanded readout remains visibly clear of the simultaneous vanilla action-bar summary. |
...
- `releaseMatrixGate -PmagSmokeSeconds=5`: passed all 22 isolated profiles on 2026-08-14 in 8m02s, including the minimal release profile, absent-mod behavior, Coasters Simulated: Track Styles, Steam 'n' Rails, Copycats+, TFMG, both Ore Excavation integrations, and Curios.
...
- Core GameTests: the final matrix passed 169/169 in 12.89s from a fresh tmpfs-backed world, including Repulsor Gun Lodestone recoil and ship impulse, the mounted-Railgun payload launch, hardened-MR-fluid chunk-reload, and magnetized-horse-armor regressions. The focused restored-feature regression namespace passed 4/4 in 341.0ms. No GameTest JVM or standard server port remained after the gate.
- Optional clients: the aggregate isolated EMI, REI, Jade, The One Probe, and Ponder-with-Steam-'n'-Rails/Copycats+ profiles all rendered the main menu with their required mods present in 1m41s. The isolated Jade and The One Probe profiles also entered singleplayer and produced the ten committed HUD captures above. The full lab client reached the world with Sodium and the pinned Iris build; the development wrapper corrects that Iris jar's exclusive `1.21.1` metadata upper bound without shipping Iris or modifying the release JAR.
