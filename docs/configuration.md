...
|---|---|---|---|---|
| CLIENT | visuals.thrusterExhaustEffectsEnabled | boolean | true | Render normal exhaust plumes and cooled Fusion mist while thrusters fire. Disable to suppress all thruster exhaust particles without changing thrust. |
...
| Scope | Key | Type | Default / range | Description |
|---|---|---|---|---|
| COMMON | propulsion.allowRedstoneThrustPower | boolean | false | Default off: a redstone signal stops FE/fuel-powered thrust machines without replacing their FE input. Enable to let a redstone signal provide their power instead. |
...
| COMMON | propulsion.fusionThrusterCoolantTankPerInterior | number | 8000; 1-1000000 | Coolant capacity per interior block. The shared tank grows with the panel. |
| COMMON | propulsion.fusionThrusterCoolantPerTickBase | number | 1; 0-10000 | Coolant consumed per firing tick before the per-cell cost. |
| COMMON | propulsion.fusionThrusterCoolantPerTickPerInterior | number | 1; 0-10000 | Additional coolant consumed per firing tick for each interior cell. |
| COMMON | propulsion.fusionThrusterCooledPowerMultiplier | number | 1.35; 1.0-10.0 | Multiplier on thrust and maximum speed while the panel consumes coolant. 1.35 means 35% more. |
| COMMON | propulsion.fusionThrusterCooledEfficiencyMultiplier | number | 1.25; 1.0-10.0 | Divides FE and propellant use while cooled. 1.25 means cooled operation uses 80% of the normal amount. |
...
| COMMON | propulsion.railgunAutoAssembleMaxBlocks | number | 0; 0-1000000 | Maximum blocks assembled into one railgun projectile. 0 = unlimited (default), 1 = a single block, 9 = a flat 3x3, and 27 = a 3x3x3 projectile. |
...
| COMMON | machines.tokamakCoolantTankPerScale | number | 8000; 1-1000000 | Coolant capacity per reactor scale step. Larger solid-core rings multiply this shared tank capacity. |
| COMMON | machines.tokamakCoolantPerTickPerScale | number | 1; 0-10000 | Coolant consumed per active tick for each reactor scale step. |
| COMMON | machines.tokamakCooledPowerMultiplier | number | 1.5; 1.0-10.0 | Multiplier on FE generation and output while the reactor consumes coolant. 1.5 means 50% more power. |
| COMMON | machines.tokamakCooledFuelEfficiency | number | 1.5; 1.0-10.0 | Multiplier on fuel-cell duration while cooled. 1.5 means each cell lasts 50% longer. |
| COMMON | machines.coolantQualityWater | number | 1.0; 0.1-10.0 | Cooling-curve quality for vanilla water. 1.0 is the baseline cooled performance and consumption. |
| COMMON | machines.coolantQualityDeuteriumOxide | number | 1.15; 0.1-10.0 | Cooling-curve quality for Deuterium Oxide. Higher values strengthen cooling bonuses and reduce coolant use. |
| COMMON | machines.coolantQualityGallium | number | 1.75; 0.1-10.0 | Cooling-curve quality for liquid Gallium, the default high-efficiency liquid-metal coolant. |
| COMMON | machines.coolantQualityTagged | number | 1.35; 0.1-10.0 | Cooling-curve quality for c:cooling_fluid entries such as TFMG Cooling Fluid. TFMG entries still honor its dedicated coolant compatibility toggle. |
...
| COMMON | compat.tfmgCoolingFluidEnabled | boolean | true | Allow TFMG Cooling Fluid to cool Tokamaks and Fusion Thrusters. When disabled, new input is rejected and already stored fluid remains inert until re-enabled. |
