...
- The machine GUI provides an explicit face map for selecting each gas output and
  the mechanical input face. Create fluid pipes connected to an assigned output
  face extract only that face's gas; the separator is intentionally not a bucket
  interface because it contains multiple gases.
